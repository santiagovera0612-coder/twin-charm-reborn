...
import {
  Sparkles, Send, ArrowRight, Check, Bot, Edit3, Play,
...
type StepId = "business" | "goal" | "tone" | "describe" | "faq" | "handoff" | "channels" | "summary";
...
const steps: StepId[] = ["business", "goal", "tone", "describe", "faq", "handoff", "channels", "summary"];

function CreateAgent() {
  const [answers, setAnswers] = useState<Answers>({});
  const [messages, setMessages] = useState<Message[]>([
    { id: "intro", role: "ai", content: <><b>¡Hola! Soy Clerivo IA.</b><br />Voy a ayudarte a crear tu agente en 5 minutos. Primero, contame: <b>¿qué tipo de negocio tenés?</b></>, step: "business" },
  ]);
  ...
  return (
    <div className="flex h-[calc(100vh-4rem)] flex-col bg-gradient-mesh">
      {/* Progress */}
      <div className="border-b border-border bg-background/80 px-6 py-4 backdrop-blur-xl">
        <div className="mx-auto max-w-3xl">
          <div className="mb-2 flex items-center justify-between text-xs">
            ...
            <span className="text-muted-foreground">Paso {Math.min(steps.indexOf(currentStep) + 1, steps.length)} de {steps.length}</span>
          </div>
      ...
      <div className="border-t border-border bg-background/80 px-6 py-5 backdrop-blur-xl">
        <div className="mx-auto max-w-3xl">
          <StepInput
            step={currentStep}
            answers={answers}
...
function aiPrompt(step: StepId, a: Answers): Message {
  const base = (content: React.ReactNode): Message => ({ id: `ai-${step}-${Date.now()}`, role: "ai", content, step });
  switch (step) {
    case "goal":
      return base(<><b>Genial, {labelOfBusiness(a.business)}.</b><br />¿Cuál es el <b>objetivo principal</b> de tu agente? Podés elegir uno o varios.</>);
    case "tone":
      return base(<>Perfecto. Ahora, <b>¿cómo querés que hable tu agente?</b> Esto define la personalidad de las respuestas.</>);
    case "describe":
      return base(<>Excelente elección. Contame ahora <b>qué vendés o qué servicio ofrecés</b>. Escribilo libremente, después te ayudo a optimizarlo ✨</>);
    case "faq":
      return base(
        <>
    ...
    case "handoff":
      return base(<>Cargado ✅. Ahora definamos cuándo derivar a un humano. <b>¿Cuándo debería pasar la conversación a una persona real?</b></>);
    case "channels":
      return base(<>Listo. Última pregunta: <b>¿en qué canales querés usar este agente?</b></>);
    case "summary":
      return base(<>🎉 <b>Tu agente está listo.</b> Acá tenés el resumen:</>);
    default:
...
function StepInput({ step, answers, onAnswer, disabled }: {
  step: StepId; answers: Answers; onAnswer: (s: StepId, v: any, d?: React.ReactNode) => void; disabled: boolean;
}) {
  const [text, setText] = useState("");
  const [multi, setMulti] = useState<string[]>([]);

  useEffect(() => { setText(""); setMulti([]); }, [step]);
      ...
      <div className="grid gap-2 sm:grid-cols-3">
        {opts.map(o => (
          <button
            ...
            className="group flex items-center gap-2.5 rounded-xl border border-border bg-card px-3 py-2.5 text-left text-sm transition hover:border-primary hover:bg-accent"
          >
  ...
  if (step === "goal") {
    const opts = ["Responder preguntas frecuentes", "Vender productos", "Agendar turnos", "Capturar leads", "Dar soporte", "Recomendar productos"];
    return <MultiChips opts={opts} multi={multi} toggle={toggle} disabled={disabled} onConfirm={() => multi.length && onAnswer("goal", multi)} />;
  }
      ...
      <div className="flex flex-wrap gap-2">
        {opts.map(o => (
          <button
            ...
            className="rounded-full border border-border bg-card px-4 py-2 text-sm font-medium transition hover:border-primary hover:bg-accent"
          >
            {o}
          </button>
        ))}
  ...
  if (step === "describe") {
    return (
      <form onSubmit={(e) => { e.preventDefault(); if (text.trim()) onAnswer("describe", text.trim()); }} className="flex gap-2">
        <input
  ...
  if (step === "faq") {
    const opts = ["Precios", "Medios de pago", "Envíos", "Horarios", "Ubicación", "Garantía", "Disponibilidad", "Cómo comprar"];
    return <MultiChips opts={opts} multi={multi} toggle={toggle} disabled={disabled} onConfirm={() => multi.length && onAnswer("faq", multi)} />;
  }

  if (step === "handoff") {
    const opts = ["Cuando quiera comprar", "Cuando pida un humano", "Cuando tenga una queja", "Cuando el agente no sepa", "Cuando sea un lead caliente"];
    return <MultiChips opts={opts} multi={multi} toggle={toggle} disabled={disabled} onConfirm={() => multi.length && onAnswer("handoff", multi)} />;
  }

  if (step === "channels") {
    const opts = ["WhatsApp", "Instagram", "Messenger", "WebChat", "Email"];
    return <MultiChips opts={opts} multi={multi} toggle={toggle} disabled={disabled} onConfirm={() => multi.length && onAnswer("channels", multi)} />;
  }
...
function MultiChips({ opts, multi, toggle, onConfirm, disabled }: {
  opts: string[]; multi: string[]; toggle: (v: string) => void; onConfirm: () => void; disabled: boolean;
}) {
  return (
    <div>
      ...
      <button
        ...
        onClick={onConfirm}
        className="inline-flex h-10 items-center gap-2 rounded-xl bg-foreground px-5 text-sm font-semibold text-background disabled:opacity-40"
      >
  ...
  return (
    <div className="rounded-2xl border border-primary/30 bg-card p-5 shadow-elegant">
      ...
      <div className="mt-5 flex flex-wrap gap-2">
        <Link to="/app/simulator" className="inline-flex h-10 items-center gap-2 rounded-xl bg-gradient-primary px-5 text-sm font-semibold text-primary-foreground shadow-glow">
